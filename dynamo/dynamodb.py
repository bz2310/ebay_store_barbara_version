...
dynamodb = boto3.resource('dynamodb',
                          aws_access_key_id="AKIA425BLJNLE5PSFXGM",
                          aws_secret_access_key="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",
...
def put_comment(table_name, commenter_name, comment, product_no):
    table = dynamodb.Table(table_name)
    dt = time.time()
    dts = time.strftime('%Y-%m-%d %H:%M:%S', time.gmtime(dt))
    res = table.put_item(Item = {"name": commenter_name,
...
def add_response(table_name, comment_id, commenter_name, response):
    table = dynamodb.Table(table_name)
    Key={
